...
export async function POST(request: Request, context: { params: { id: string } }) {
  ...
  await supabaseAdmin.from("table_requests").update({ status: "forwarded" }).eq("id", context.params.id);

  return NextResponse.redirect(new URL(`${redirectTo}?status=forwarded`, request.url));
}
