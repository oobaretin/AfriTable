...
export function RestaurantCardWithDistance({ restaurant, distance }: RestaurantCardWithDistanceProps) {
  ...
  const trustBase = evaluateCatalogTrust({
    ...
    hours: (restaurant as CatalogListItem).hours,
  ...
  const trust =
    (restaurant as CatalogListItem).trust_level === "vetted"
      ? { ...trustBase, level: "vetted" as const, label: "Vetted dine-in", shortLabel: "Vetted" }
      : trustBase;
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {trust.level === "vetted" ? (
              <CatalogTrustBadge
                trust={trust}
                size="sm"
                className="border-brand-forest/40 bg-brand-forest/20 text-[#9BE59B]"
              />
            ) : null}
            <BookingStatusBadge
              action={bookingAction}
              size="sm"
              className="border-white/20 bg-white/10 text-[#C69C2B]"
            />
          </div>
        </div>
        {restaurant.rating && (
      ...
      <div className="flex gap-3 mt-6">
        ...
        {restaurant.website && (
          <a
            ...
            className="flex items-center justify-center gap-2 bg-[#C69C2B] hover:bg-[#C69C2B]/90 text-[#050A18] text-sm font-semibold px-4 py-2.5 rounded-lg transition-colors relative z-10 cursor-pointer"
          >
            Visit Website
